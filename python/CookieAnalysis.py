...
if __name__ == "__main__":
    ...
    variance1 = statistics.variance(times_avg)
    q125 = np.quantile(times_avg, 0.25)
    q150 = np.quantile(times_avg, 0.5)
    q175 = np.quantile(times_avg, 0.75)
    ...
    plt.legend()
    plt.show()
    plt.savefig("Times_HistogramPlot.png")
    ...
    plt.axvline(Average(times_avg), color='b',linestyle='solid', linewidth=1, label ='Times_avg Average')
    plt.title('Cookie Time Probability')
    plt.xlabel('Time')
    plt.ylabel('Probability')
    plt.legend()
    plt.show()
    plt.savefig("Times_avg_HistogramPlot.png")
